...
export const fetchCavesByGeolocation = async (swLat, swLng, neLat, neLng) => {
    try {
        const response = await axios.get(`${API_BASE_URL}/api/cave/coordinates/geolocation?sw_lat=${swLat}&sw_lng=${swLng}&ne_lat=${neLat}&ne_lng=${neLng}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
...
// Function to fetch caves by geolocation coordinates
export const fetchCaveDataByGeolocation = async (swLat, swLng, neLat, neLng) => {
    try {
        const response = await axios.get(`${API_BASE_URL}/api/cave/data/geolocation?sw_lat=${swLat}&sw_lng=${swLng}&ne_lat=${neLat}&ne_lng=${neLng}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
